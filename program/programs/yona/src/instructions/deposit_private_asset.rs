...
pub struct DepositPrivateAsset<'info> {
    #[account(
        mut,
        seeds = [b"merkle_tree"],
...
pub fn handler<'info>(
    ...
    // For private asset deposit, recipient is the private_asset_config PDA (no reserve needed)
    let ext_data = ExtData::from_minified(
        &ctx.accounts.private_asset_config.key(),
    ...
    // Burn tokens on deposit (no reserve needed for private assets)
